...
class TestGroupCreate:
    ...
    @pytest.mark.asyncio
    async def test_create_group_duplicate_name(self, admin_client: AsyncClient, test_group):
        group_data = {
...
class TestGroupGet:
    ...
    async def test_get_nonexistent_group(self, admin_client: AsyncClient):
        resp = await admin_client.get("/api/v1/groups/00000000-0000-0000-0000-000000000000")
...
class TestGroupList:
    ...
    @pytest.mark.asyncio
    async def test_list_groups_filter_by_name(self, admin_client: AsyncClient, test_group):
    ...
    @pytest.mark.asyncio
    async def test_list_groups_filter_by_is_system(self, admin_client: AsyncClient, system_group):
...
class TestGroupUpdate:
    ...
    async def test_update_group_success(self, admin_client: AsyncClient, test_group):
        ...
        resp = await admin_client.patch(f"/api/v1/groups/{test_group.id}", json=update_data)
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
...
class TestGroupDelete:
    ...
    @pytest.mark.asyncio
    async def test_delete_group_success(self, admin_client: AsyncClient, async_db_session):
        ...
        resp = await admin_client.delete("/api/v1/groups/00000000-0000-0000-0000-000000000001")
        assert resp.status_code == status.HTTP_204_NO_CONTENT

    @pytest.mark.asyncio
    async def test_delete_system_group_forbidden(self, admin_client: AsyncClient, system_group):
    ...
    async def test_delete_nonexistent_group(self, admin_client: AsyncClient):
        resp = await admin_client.delete("/api/v1/groups/00000000-0000-0000-0000-000000000000")
        assert resp.status_code == status.HTTP_404_NOT_FOUND
...
class TestGroupUserManagement:
    ...
    @pytest.mark.asyncio
    async def test_add_user_to_group(self, admin_client: AsyncClient, regular_user, test_group):
    ...
    @pytest.mark.asyncio
    async def test_remove_user_not_in_group(self, admin_client: AsyncClient, regular_user, test_group):
        resp = await admin_client.delete(
            f"/api/v1/groups/{test_group.id}/users/{regular_user.id}"
        )
        assert resp.status_code == status.HTTP_404_NOT_FOUND
...
class TestUserGroups:
    ...
    @pytest.mark.asyncio
    async def test_list_user_groups(self, admin_client: AsyncClient, user_in_group, test_group):
    ...
    @pytest.mark.asyncio
    async def test_list_my_groups(self, authenticated_client: AsyncClient, user_in_group, test_group):
        resp = await authenticated_client.get("/api/v1/groups/me/groups")
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
        assert "items" in data
        assert len(data["items"]) >= 1
